...
if mod1 == 'GEX':
    ...
    if mod2 == 'ADT':
        y_pred = []
        model = torch.load(os.path.join(par['input_pretrain'], 'f_alpha_conv4_mean_fullbatch_12000_phase2_inductive_batch_speration.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_10000_phase2_inductive_gex2adt_2.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_12000_phase2_inductive_gex2adt_sep_2.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_15000_phase2_inductive.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        ...
        y_pred = []
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_8000_phase2_inductive_gex2atac_3.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_8000_phase2_inductive_gex2atac_2.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_8000_phase2_inductive_gex2atac.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
        
        model = torch.load(os.path.join(par['input_pretrain'], 'bf_alpha_conv4_mean_fullbatch_10000_phase2_inductive_gex2atac.pkl'), map_location=torch.device('cpu'))
        y_pred.append(evaluate(model).numpy())
